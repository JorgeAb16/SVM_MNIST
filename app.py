...
def procesar_trazo_dibujado(image_data):
    """Convierte el arreglo RGBA del canvas (trazo oscuro sobre fondo claro) en un
    vector de 784 píxeles compatible con el formato de entrenamiento (estilo MNIST:
    trazo claro sobre fondo oscuro, valores 0-255)."""
    img = Image.fromarray(image_data.astype("uint8"), mode="RGBA").convert("L")
    img = img.resize((28, 28), Image.LANCZOS)
    arr = np.array(img).astype("float32")
    arr_invertido = 255.0 - arr  # el canvas dibuja oscuro sobre claro; MNIST es al revés
...
# Tabs
# ----------------------------------------------------------------------------
tab_pca, tab_kmeans, tab_svm, tab_dibujo, tab_comp = st.tabs(
    ["🧬 PCA", "📊 K-Means", "🧩 Clasificación (SVM)", "✏️ Dibuja un dígito", "📈 Comparación"]
)
...
# --- Tab Dibujar dígito ------------------------------------------------------
with tab_dibujo:
    st.subheader(f"Dibuja un dígito (0-9) y clasifícalo con {n_components} componentes PCA")
 
    if not CANVAS_DISPONIBLE:
        st.error(
            "Falta instalar `streamlit-drawable-canvas`. Agrega `streamlit-drawable-canvas` "
            "a tu `requirements.txt` y reinstala las dependencias."
        )
    else:
        col1, col2 = st.columns([1, 1])
        with col1:
            st.caption("Dibuja con el mouse o el dedo (pantalla táctil) dentro del recuadro.")
            ...
            clasificar_dibujo = btn_col1.button("🔍 Clasificar dibujo", use_container_width=True, type="primary")
            btn_col2.caption("Usa el ícono 🗑️ de la esquina del lienzo para borrar y volver a dibujar.")
        ...
        with col2:
            ...
            if clasificar_dibujo and hay_trazo:
                ...
                st.markdown("**Lo que el modelo \"ve\" (28x28, tras preprocesar):**")
                ...
                st.markdown(
                    ...
                    <div class="pred-card">
                        ...
                        <div style="margin-top:0.5rem; font-size:0.9rem;">
                            Cluster K-Means asignado: {cluster_asignado}
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                ...
                st.warning("El lienzo está vacío. Dibuja un dígito antes de clasificar.")
            else:
                st.info("Dibuja un dígito y presiona **Clasificar dibujo** para ver la predicción.")
 
        st.caption(
            "Nota: el SVM fue entrenado con imágenes del dataset MNIST (dígitos centrados, "
            "trazo uniforme). Un dibujo hecho a mano libre puede lucir distinto, así que la "
            "predicción no siempre será perfecta — es un buen ejemplo de cómo el modelo "
            "generaliza (o no) fuera de su distribución de entrenamiento."
        )
